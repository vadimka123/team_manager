...
class TaskListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        if self.request.user.account_type in (USER_TYPE_TEAM_CHIEF, USER_TYPE_TEAM_LEADER):
            return Task.objects.all().select_related('user_created', 'user_dev')
        else:
            return Task.objects.filter(user_dev=self.request.user).select_related('user_created', 'user_dev')
